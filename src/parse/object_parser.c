...
/*   Updated: 2025/09/15 18:14:23 by mmariano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_sphere(char **tokens, t_scene *scene, int line_number)
{
	...
	if (count_tokens(tokens) != 4)
		parse_error("Invalid sphere parameters", line_number);
		
	sphere = create_sphere();
	sphere->origin = string_to_vector(tokens[1], 1.0);
	diameter = ft_atof(tokens[2]);
	if (diameter <= 0.0)
		parse_error("Sphere diameter must be greater than 0", line_number);
...
void	parse_plane(char **tokens, t_scene *scene, int line_number)
{
	...
	if (count_tokens(tokens) != 4)
		parse_error("Invalid plane parameters", line_number);
	...
	if (normal.x < -1.0 || normal.x > 1.0 || normal.y < -1.0
		...
		parse_error("Plane normal vector values must be in range [-1.0, 1.0]", line_number);
	plane->prop.plane.normal = normal;
	set_transform(plane, translation(plane->origin.x, plane->origin.y, plane->origin.z));
...
void	parse_cylinder(char **tokens, t_scene *scene, int line_number)
{
	...
	if (count_tokens(tokens) != 6)
		parse_error("Invalid cylinder parameters", line_number);
	...
	if (!cylinder)
		parse_error("Memory allocation failed for a new cylinder", line_number);
	...
	if (diameter <= 0.0 || height <= 0.0)
		parse_error("Cylinder diameter and height must be greater than 0", line_number);
